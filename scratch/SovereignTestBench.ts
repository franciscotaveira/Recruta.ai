...
async function runTest() {
  ...
  const { data: jobFail, error: jobFailErr } = await supabase.from('system_jobs').insert({
    user_id: TEST_RECRUITER_ID,
    action_id: 'recruiter.invite_candidate',
    payload: { phone: '5511999999999', name: 'Candidato Teste', jobId: TEST_JOB_ID, jobTitle: 'Dev', companyName: 'MCT' },
    status: 'queued'
  }).select().single();
  ...
  // Trigger another job
  const { data: jobPass } = await supabase.from('system_jobs').insert({
    user_id: TEST_RECRUITER_ID,
    action_id: 'recruiter.invite_candidate',
    payload: { phone: '5511999999999', name: 'Candidato Teste 2', jobId: TEST_JOB_ID, jobTitle: 'Senior Dev', companyName: 'MCT' },
    status: 'queued'
  }).select().single();
