  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-fade-in">
      ...
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400 text-sm">Gerencie seus créditos e configurações de pagamento.</p>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
              ...
              <div className="bg-slate-900 dark:bg-purple-900 rounded-2xl p-8 text-white shadow-2xl relative overflow-hidden">
                  ...
                  <div className="relative z-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
                      <div>
                          <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-2">Saldo Disponível</p>
                          ...
                          <p className="text-xs text-slate-400 mt-4 flex items-center gap-2">
                             ...
                             Você tem créditos suficientes para aprox. <strong>{Math.floor(wallet.balance)} triagens</strong>.
                          </p>
                      </div>
                      ...
                      <div className="bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/10 min-w-[200px]">
                          <div className="flex items-center justify-between mb-3">
                              ...
                              >
                                  <div className={`absolute top-1 w-3 h-3 bg-white rounded-full shadow-sm transition-all ${autoRecharge ? 'right-1' : 'left-1'}`}></div>
                              </div>
                          </div>
                          {autoRecharge ? (
                              <p className="text-[10px] text-slate-300 leading-snug">
                                  Recarregar <strong>{wallet.autoRechargeAmount} créditos</strong> quando saldo for menor que <strong>{wallet.autoRechargeThreshold}</strong>.
                              </p>
                          ) : (
              ...
              {/* CREDIT PACKAGES */}
              <div>
                  <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Adicionar Créditos</h3>
                  <div className="grid md:grid-cols-3 gap-4">
                      {CREDIT_PACKAGES.map(pkg => (
                          <div key={pkg.id} className={`relative bg-white dark:bg-slate-800 border-2 rounded-xl p-6 cursor-pointer hover:border-purple-500 transition-all group ${pkg.bestValue ? 'border-purple-500 shadow-lg' : 'border-slate-200 dark:border-slate-700'}`}>
                              {pkg.bestValue && (
                              ...
                              <div className="text-center mb-4">
                                  <p className="text-slate-500 dark:text-slate-400 text-xs font-bold uppercase mb-1">{pkg.name}</p>
                                  <p className="text-3xl font-black text-slate-900 dark:text-white">{pkg.credits} <span className="text-sm font-medium text-slate-400">cr</span></p>
                              </div>
                          ...
                          <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                              {wallet.transactions.map(tx => (
                                  <tr key={tx.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                                      ...
                                      <td className="px-6 py-3 font-medium text-slate-900 dark:text-white">{tx.description}</td>
                                      <td className={`px-6 py-3 text-right font-bold ${tx.type === 'credit' ? 'text-green-600' : 'text-slate-600 dark:text-slate-400'}`}>
          ...
          <div className="space-y-6">
              <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
                  ...
                  <ul className="space-y-3 text-sm text-slate-600 dark:text-slate-400">
                      <li className="flex items-start gap-2">
                          ...
                          <span>Créditos são descontados apenas quando o candidato <strong>inicia</strong> a triagem.</span>
                      </li>
                      <li className="flex items-start gap-2">
                          <div className="w-1.5 h-1.5 rounded-full bg-slate-400 mt-1.5"></div>
              ...
              <div className="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-6 border border-dashed border-slate-300 dark:border-slate-700 text-center">
                  <p className="text-sm font-bold text-slate-900 dark:text-white mb-2">Precisa de Recibos?</p>
                  <p className="text-xs text-slate-500 mb-4">Todas as faturas são enviadas automaticamente para o e-mail cadastrado (financeiro@techcorp.com).</p>
