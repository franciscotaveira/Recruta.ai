...
import { Search, Filter, Download, X, CheckCircle, XCircle, User, FileText, Zap, MessageCircle, AlertTriangle, Lock, Eye } from 'lucide-react';
    ...
    <div className="space-y-6 relative h-full animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm">Visualize diagnósticos. <span className="text-purple-600 font-bold">Ativar um candidato consome 1 crédito.</span></p>
        </div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              {filteredCandidates.map((candidate) => (
                ...
                >
                        ...
                        <div>
                            ...
                            <div className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1">
                                {candidate.isActivated ? (
                                    <span className="text-emerald-500 flex items-center gap-0.5"><CheckCircle size={10} /> Ativo</span>
                                ) : (
                                    <span className="text-slate-400 flex items-center gap-0.5"><Lock size={10} /> Contato Oculto</span>
                                )}
                            </div>
                        </div>
                    </div>
                  ...
                  <td className="px-6 py-4">
                    {candidate.score > 0 ? (
                      <span className={`font-bold ${candidate.score >= 80 ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'}`}>
                        {candidate.score}/100
                      </span>
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium capitalize ${
                      candidate.status === 'completed' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300' :
                      candidate.status === 'processing' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300' :
        ...
        <div className="fixed inset-y-0 right-0 w-full md:w-[480px] bg-white dark:bg-slate-900 shadow-2xl border-l border-slate-200 dark:border-slate-700 transform transition-transform duration-300 overflow-y-auto z-40 animate-fade-in-up">
           <div className="p-6 pb-24">
              ...
              <div className="flex justify-between items-start mb-6">
                 ...
                 <button onClick={() => setSelectedCandidate(null)} className="text-slate-400 hover:text-slate-700 dark:hover:text-slate-200">
                    <X size={24} />
                 </button>
              </div>
              ...
              {!selectedCandidate.isActivated ? (
                  <div className="bg-slate-900 dark:bg-slate-800 p-4 rounded-xl text-white mb-6 border border-slate-700 shadow-lg">
                      ...
                      <p className="text-xs text-slate-300 mb-4">
                          Para acessar contatos (WhatsApp) e iniciar triagem, você deve ativar este candidato.
                      </p>
                      <button 
              ...
              <div className={`space-y-3 mb-6 ${!selectedCandidate.isActivated ? 'blur-sm select-none opacity-50 pointer-events-none' : ''}`}>
                  <div className="flex items-center gap-3 text-sm">
                      <MessageCircle size={16} className="text-green-500" />
                      <span className="font-medium">{selectedCandidate.phone || "+55 11 99999-9999"}</span>
                  </div>
                  <div className="flex items-center gap-3 text-sm">
                      <FileText size={16} className="text-blue-500" />
                      <span className="font-medium">{selectedCandidate.email || "candidato@email.com"}</span>
                  </div>
              </div>
              ...
              <div className="bg-purple-50 dark:bg-purple-900/10 border border-purple-100 dark:border-purple-800/30 p-4 rounded-xl mb-6">
                 ...
                 <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed">
                    {selectedCandidate.diagnosis || "Análise comportamental pendente. O candidato mostra fortes indícios técnicos no currículo."}
                 </p>
              </div>
              ...
              <div className="mb-6">
                 <div className="flex justify-between items-center mb-4">
                    <h3 className="font-bold text-slate-900 dark:text-white">Score de Clareza (SCPD)</h3>
                    <span className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{selectedCandidate.score}/100</span>
                 </div>
                 <div className="space-y-3 bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700">
                    ...
                    <div className="flex justify-between items-center text-sm">
                       <span className="text-slate-600 dark:text-slate-400">Evidência de resultados</span>
