      ...
      >
          ...
          <div className="bg-slate-50 dark:bg-[#1A1A2E] rounded-xl p-4 border border-slate-200 dark:border-[#2D2D44]">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-medium text-slate-900 dark:text-white truncate text-sm">Francisco T.</p>
                <p className="text-xs text-gray-500 truncate">Ciclo Ativo</p>
              </div>
            </div>
            <div className="mt-3 flex items-center justify-between">
              <span className="text-xs text-gray-400">Score SCPD</span>
              <span className="text-xs font-bold text-emerald-500 dark:text-emerald-400">87/100</span>
            </div>
        ...
        <nav className="flex-1 px-3 space-y-1">
          {menuItems.map((item) => (
            ...
            >
              <span className={`mr-3 ${isActive(item.path) ? 'text-purple-600 dark:text-purple-400' : 'text-slate-400 group-hover:text-slate-900 dark:group-hover:text-white'}`}>
                {item.icon}
              </span>
              {item.label}
            </Link>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <div className="hidden lg:flex justify-end px-8 pt-4">
           <button 
            ...
            >
              {isDarkMode ? <><Sun size={16} /> Modo Claro</> : <><Moon size={16} /> Modo Escuro</>}
            </button>
        </div>
