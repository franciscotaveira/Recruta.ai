...
export const GovernanceQueue: React.FC = () => {
  ...
  if (loading)
    return <div className="p-8 text-center text-slate-500">Carregando fila de governança...</div>;
      ...
      {pending.length === 0 ? (
        <div className="bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-200 dark:border-emerald-800/50 rounded-2xl p-8 text-center">
          ...
          <p className="text-emerald-800 dark:text-emerald-300 font-medium">
            Nenhuma ação pendente de revisão humana.
          </p>
        </div>
      ) : (
        <div className="grid gap-4">
          {pending.map((job) => {
            ...
            return (
              <div
                key={job.id}
                className="bg-white dark:bg-slate-900 border-2 border-red-500/20 rounded-2xl p-5 shadow-sm"
              >
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-sm font-black text-slate-900 dark:text-white">
                      {metadata?.estimated_cost || 0} créditos
                    </p>
                  </div>
                </div>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      {job.status === 'completed' ? (
                        ...
                        <span className="text-red-500 text-xs truncate max-w-[100px] inline-block">
                          {job.error}
                        </span>
                      ) : (
