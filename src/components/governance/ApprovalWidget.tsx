    ...
    const unsubscribe = JobQueue.subscribe((updatedJob) => {
      setPendingJobs((prev) => {
        ...
        if (updatedJob.status === 'pending_approval') {
          return [...filtered, updatedJob].sort(
    ...
    <div className="fixed bottom-4 right-4 w-96 z-50 flex flex-col gap-3">
      {pendingJobs.map((job) => {
        ...
        return (
          <div
            key={job.id}
            className="bg-slate-900/80 backdrop-blur-md border border-red-500/30 rounded-xl p-4 shadow-2xl animate-in slide-in-from-bottom-5"
          >
