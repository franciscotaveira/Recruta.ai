        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900 dark:text-white flex items-center gap-2">
              {job.title}
              <span className="text-xs font-normal text-slate-500 border border-slate-200 dark:border-slate-700 px-2 py-0.5 rounded-full">
                Remote
              </span>
            </h1>
      ...
      <div className="flex-1 overflow-x-auto overflow-y-hidden">
        <div className="flex h-full gap-4 min-w-max pb-4">
          {stages.map((stage) => (
            <div
              key={stage.id}
              className="w-72 flex flex-col bg-slate-100 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-800"
            >
              {/* Column Header */}
              <div
              ...
              <div className="p-2 flex-1 overflow-y-auto space-y-2 custom-scrollbar">
                {mockCards
                  ...
                  .map((card) => (
                    <div
                      key={card.id}
                      className="bg-white dark:bg-slate-800 p-3 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 cursor-move hover:shadow-md transition-shadow group"
                    >
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <span
                          className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${card.score >= 90 ? 'bg-emerald-100 text-emerald-700' : 'bg-yellow-100 text-yellow-700'}`}
                        >
                          Match: {card.score}%
                        </span>
