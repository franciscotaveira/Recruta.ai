...
const RecruiterDashboard = () => {
  ...
  const handleConfirmBulk = () => {
    ...
    if (success) {
      ...
      setUploadFile(null);
      alert(
        `Sucesso! ${activationQuantity} candidatos foram enviados para triagem e o valor foi debitado.`
      );
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-800 p-4 rounded-xl">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-xs font-bold uppercase text-yellow-800 dark:text-yellow-500">
                    Custo da Operação
                  </span>
                  <span className="font-black text-xl text-slate-900 dark:text-white">
                    {activationQuantity} Créditos
                  </span>
                </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium">
            Inteligência Artificial processou 12 novos perfis hoje.
          </p>
        </div>
        <div className="flex gap-3">
          ...
          <Link
            ...
            className="hidden sm:flex items-center gap-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-4 py-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors cursor-pointer group"
          >
            <div className="text-right">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider group-hover:text-purple-500 transition-colors">
                Saldo Decisório
              </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
          <div className="flex justify-between items-center mb-4">
            <span className="text-slate-500 dark:text-slate-400 text-xs font-black uppercase tracking-widest">
              Processos Ativos
            </span>
                ...
                <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 font-medium">
                      {job.location} • Criado em {job.postedDate}
                    </p>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                ...
                <div className="bg-slate-50 dark:bg-slate-900/40 p-6">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {[
                      ...
                      <div
                        key={i}
                        className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 group hover:border-purple-500 cursor-pointer transition-all shadow-sm"
                      >
                        <div className="flex items-center gap-3 mb-3">
                          <div className="w-8 h-8 rounded-lg bg-slate-900 dark:bg-purple-600 text-white flex items-center justify-center font-black text-[10px]">
                            {cand.initial}
                          </div>
                          <div>
                            <p className="text-xs font-black text-slate-900 dark:text-white truncate group-hover:text-purple-600 transition-colors">
                              {cand.name}
                            </p>
                        ...
                        <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 group-hover:text-slate-600 dark:group-hover:text-slate-200">
                          {cand.status}
                        </span>
                      </div>
                    ))}
        ...
        <div className="space-y-6">
          <div className="bg-slate-900 dark:bg-purple-900 rounded-2xl shadow-2xl p-6 text-white relative overflow-hidden border border-slate-700 dark:border-purple-800">
            ...
            <p className="text-indigo-100 text-xs mb-6 relative z-10 font-medium leading-relaxed">
              Transforme seus PDFs antigos em candidatos ativos.{' '}
              <span className="text-white font-bold">Importe e pague apenas por quem ativar.</span>
            </p>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6">
            <h3 className="font-black text-slate-900 dark:text-white mb-6 uppercase text-[10px] tracking-widest text-slate-400">
              Atalhos
            </h3>
            <div className="space-y-3">
              <button className="w-full flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition-all group">
                <span className="flex items-center gap-3">
                  <Briefcase size={18} className="text-slate-400" /> Novo Processo
                </span>
                <ChevronRight
                  size={16}
              ...
              <button className="w-full flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition-all group">
                <span className="flex items-center gap-3">
                  <Search size={18} className="text-slate-400" /> Buscar Talentos
                </span>
                <ChevronRight
                  size={16}
                  className="text-slate-300 group-hover:translate-x-1 transition-transform"
