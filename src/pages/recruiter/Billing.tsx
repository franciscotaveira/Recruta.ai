...
import { Zap, Clock, ExternalLink, Loader2, Shield, ArrowUpRight, DollarSign, CheckCircle2, Star, Building2, AlertTriangle } from 'lucide-react';
...
const RecruiterBilling = () => {
  const { user } = useAuth();
  const { wallet, profile, transactions, loading, error, refresh, buyCredits, buySubscription } = useBilling();
  ...
  const handleBuyCredits = async (packageId: string) => {
    setBuying(packageId);
    try {
      const customer = phone.trim() && taxId.trim() ? { name: user?.name || '', email: user?.email || '', phone: phone.trim(), taxId: taxId.trim() } : undefined;
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 pb-16 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white">Assinatura & Créditos</h1>
      ...
      {wallet && wallet.trigger_balance < 10 && (
        <div className="bg-amber-500/10 border border-amber-500/20 p-4 rounded-2xl flex items-center gap-4 animate-pulse">
          ...
          <div>
            <p className="text-sm font-black text-amber-600 uppercase tracking-tight">Saldo de Disparos Crítico</p>
            <p className="text-xs text-amber-600/80 font-medium">Seu saldo está abaixo de 10 créditos. Recarregue para evitar que novas entrevistas sejam bloqueadas.</p>
          </div>
        </div>
      )}
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {/* Status Card */}
        <div className={`col-span-1 p-6 rounded-3xl border-2 transition-all ${isSubscribed ? 'bg-indigo-50/50 dark:bg-indigo-900/10 border-indigo-200 dark:border-indigo-800' : 'bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700'}`}>
           <div className="flex items-center gap-3 mb-4">
             <div className={`p-2 rounded-xl ${isSubscribed ? 'bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400' : 'bg-slate-200 dark:bg-slate-700 text-slate-500'}`}>
             ...
             <div>
               ...
               <p className="font-bold text-slate-900 dark:text-white">{isSubscribed ? profile.subscription_plan?.toUpperCase() : 'FREE (Inativo)'}</p>
             </div>
           </div>
           {isSubscribed ? (
             <div className="space-y-1">
               <p className="text-xs text-slate-500">Status: <span className="text-emerald-500 font-bold">ATIVO</span></p>
               <p className="text-xs text-slate-500 text-balance">Expira em: {new Date(profile.subscription_expires_at).toLocaleDateString('pt-BR')}</p>
             </div>
           ) : (
             <p className="text-xs text-slate-500 leading-relaxed">Sua conta está no modo limitado. Ative o Plano Pro para liberar a triagem IA ilimitada.</p>
           )}
        </div>
        ...
        <div className="col-span-1 md:col-span-2 bg-gradient-to-br from-slate-900 to-indigo-950 rounded-3xl p-6 text-white shadow-xl relative overflow-hidden">
          ...
          <div className="relative z-10 flex items-center justify-between">
            <div>
              <p className="text-indigo-300 text-[10px] font-black uppercase tracking-widest mb-1">Saldo de Disparos WhatsApp</p>
            ...
            <div className="text-right">
              ...
              <p className="text-xs font-medium text-indigo-200 max-w-[200px]">Use para convidar candidatos para entrevistas por voz automatizadas.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-slate-400 uppercase ml-1">WhatsApp para Notificações</label>
            <input
              value={phone}
      ...
      {!isSubscribed && (
        <section>
          <div className="text-center mb-8">
            <h2 className="text-2xl font-black text-slate-900 dark:text-white">Assinatura de Inteligência</h2>
            <p className="text-slate-500 text-sm">Libere o cérebro do Recrutaria para sua empresa.</p>
          </div>
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {SUBSCRIPTION_PLANS.map((plan) => (
              <div key={plan.id} className={`relative p-8 rounded-3xl border-2 bg-white dark:bg-slate-900 transition-all hover:scale-[1.02] ${plan.bestValue ? 'border-indigo-500 shadow-indigo-500/10 shadow-2xl' : 'border-slate-200 dark:border-slate-800'}`}>
                {plan.bestValue && (
                ...
                <div className="mb-8">
                  <div className="flex items-baseline gap-1">
                    ...
                    <span className="text-4xl font-black text-slate-900 dark:text-white">{plan.price.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
      ...
      <section>
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-2xl font-black text-slate-900 dark:text-white">Recarga de Disparos</h2>
            <p className="text-slate-500 text-sm">Créditos avulsos para convites ativos via WhatsApp.</p>
          </div>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {PACKAGES.map((pkg) => (
            <div key={pkg.id} className={`group p-6 rounded-3xl border-2 bg-white dark:bg-slate-900 transition-all hover:border-indigo-400 ${pkg.bestValue ? 'border-indigo-100 dark:border-indigo-900/30' : 'border-slate-100 dark:border-slate-800'}`}>
               <div className="flex justify-between items-start mb-4">
                 <div className="p-2 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 rounded-xl">
                   <Zap size={16} fill="currentColor" />
                 </div>
                 {pkg.bestValue && <span className="text-[8px] font-black bg-indigo-100 text-indigo-700 px-2 py-1 rounded-full uppercase">Melhor Oferta</span>}
               </div>
      ...
      <div className="flex items-center gap-3 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700">
        ...
        <p className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold tracking-tight">
          Pagamento processado com segurança pela <strong className="text-indigo-600">AbacatePay</strong>. Créditos de disparo não expiram.
        </p>
      </div>

      {/* Transaction History */}
      <div>
        <h2 className="text-xl font-black text-slate-900 dark:text-white mb-6">Histórico Recente</h2>
            ...
            {(transactions || []).map((tx) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`p-2.5 rounded-xl ${tx.type === 'purchase' ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600' : 'bg-red-50 dark:bg-red-900/20 text-red-600'}`}>
                ...
                <div className="text-right">
                  <p className={`text-sm font-black ${tx.type === 'purchase' ? 'text-emerald-600' : 'text-red-600'}`}>
                    {tx.type === 'purchase' ? '+' : '-'}{tx.amount} CR
                  </p>
