...
const LandingPage = () => {
      ...
      ? [
          {
            ...
            answer:
              'NÃO. Somos uma ferramenta de preparação técnica. Garantimos que seu currículo passará pelos robôs (ATS), mas a contratação depende de você.',
          },
          {
            question: 'Por que pagar R$ 49 se existem modelos grátis?',
            answer:
              'Modelos grátis são apenas design. Nós somos engenharia de conteúdo. A IA reescreve suas experiências com as palavras-chave exatas que os recrutadores buscam.',
          },
          {
            question: 'As vagas internas são reais?',
            answer:
              'Sim, são um BÔNUS. Empresas parceiras usam nossa IA para triagem. Se der match, você aparece para elas. Mas o foco é te preparar para vagas de fora.',
          },
        ]
      : [
          {
            ...
            answer:
              "Você não paga para anunciar vaga nem para ver a lista de aplicantes. Você gasta 1 crédito apenas quando decide 'Destravar' um candidato para ver o contato ou convidá-lo.",
          },
          {
            question: 'A IA substitui o recrutador?',
            answer:
              'Não. A IA elimina o trabalho braçal de ler 500 PDFs ruins. Ela entrega o shortlist dos 10 melhores para você entrevistar.',
          },
          {
            question: 'Posso importar currículos que recebi por e-mail?',
            answer:
              "Sim! Use nossa função 'Reaquecimento de Base'. Suba os PDFs antigos, a IA analisa, classifica e você decide quem ativar.",
          },
        ];
  ...
  return (
    <div
      className={`min-h-screen bg-white font-sans text-slate-900 scroll-smooth transition-colors duration-500`}
    >
      {/* 1. ANNOUNCEMENT BAR */}
      <div className="bg-slate-900 text-white py-2 px-4 text-xs md:text-sm font-medium text-center overflow-hidden whitespace-nowrap relative z-50">
        <div
          className={`transition-opacity duration-500 flex items-center justify-center gap-2 ${fade ? 'opacity-100' : 'opacity-0'}`}
        >
          <span
            className={`w-2 h-2 rounded-full animate-pulse ${activeTab === 'candidate' ? 'bg-purple-400' : 'bg-emerald-400'}`}
          ></span>
          {currentNotification}
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <div className="flex items-center gap-2">
              <div
                className={`w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-xl transition-colors duration-500 ${activeTab === 'candidate' ? 'bg-slate-900' : 'bg-slate-800'}`}
              >
                {activeTab === 'candidate' ? (
                  <Mic size={20} />
                ) : (
      ...
      <div
        className={`relative pt-20 pb-32 overflow-hidden ${theme.bg} transition-colors duration-500`}
      >
        {/* Background Decorative Blobs */}
        <div
          className={`absolute top-0 right-0 w-[500px] h-[500px] rounded-full mix-blend-multiply filter blur-[128px] opacity-20 animate-pulse-slow transition-colors duration-500 ${activeTab === 'candidate' ? 'bg-purple-300' : 'bg-emerald-300'}`}
        ></div>
        <div
          className={`absolute top-0 left-0 w-[500px] h-[500px] rounded-full mix-blend-multiply filter blur-[128px] opacity-20 animate-pulse-slow animation-delay-2000 transition-colors duration-500 ${activeTab === 'candidate' ? 'bg-blue-300' : 'bg-teal-300'}`}
        ></div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white border border-slate-200 text-slate-600 text-xs font-bold mb-8 shadow-sm hover:shadow-md transition-all cursor-default animate-fade-in-up">
            <span
              className={`w-2 h-2 rounded-full ${activeTab === 'candidate' ? 'bg-purple-500' : 'bg-emerald-500'}`}
            ></span>
            <span>
              {activeTab === 'candidate'
                ? 'Engenharia de Carreira'
                : 'Inteligência de Decisão para RH'}
            </span>
          </div>

          {/* Dynamic Headline */}
          <h1
            className="text-5xl md:text-7xl font-extrabold text-slate-900 tracking-tight mb-8 leading-[1.1] max-w-5xl mx-auto animate-fade-in-up transition-all duration-300"
          ...
          {/* Dynamic Subhead */}
          <p
            className="mt-6 max-w-2xl mx-auto text-xl text-slate-600 mb-10 leading-relaxed animate-fade-in-up"
          ...
          {/* Social Proof Text */}
          <div
            className="mt-12 flex items-center justify-center gap-6 text-sm text-slate-400 font-medium grayscale opacity-70 animate-fade-in-up"
            style={{ animationDelay: '0.5s' }}
          >
            <span>
              {activeTab === 'candidate' ? 'Candidatos aprovados em:' : 'Empresas que usam:'}
            </span>
      ...
      <div
        id="como-funciona"
        className="py-24 bg-white border-b border-slate-100 transition-colors duration-500 relative z-20"
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-12 relative">
            ...
            <div
              className={`hidden md:block absolute top-12 left-[16%] right-[16%] h-1 bg-gradient-to-r ${theme.gradient} opacity-20 -z-10 rounded-full`}
            ></div>

            {/* Step 1 */}
            <div className="flex flex-col items-center text-center group">
              <div
                className={`w-24 h-24 rounded-3xl flex items-center justify-center mb-6 shadow-xl transition-all duration-300 group-hover:-translate-y-2 group-hover:scale-105 ${theme.iconBg} ${theme.text}`}
              >
                {activeTab === 'candidate' ? <Upload size={40} /> : <Briefcase size={40} />}
              </div>
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold mb-4 shadow-md ${theme.button}`}
              >
                1
              </div>
      ...
      <div
        id="problema"
        className="py-24 bg-slate-900 text-white relative overflow-hidden transition-colors duration-500"
      >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid md:grid-cols-2 gap-16 items-center">
            ...
            <div>
              <h2
                className={`font-bold uppercase tracking-widest text-sm mb-4 ${activeTab === 'candidate' ? 'text-purple-400' : 'text-emerald-400'}`}
              >
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <div className="w-full bg-slate-700 h-8 rounded relative overflow-hidden">
                        <div className="absolute top-0 left-0 h-full bg-slate-600 w-full flex items-center px-3 text-xs text-slate-300">
                          300 Inscritos
                        </div>
                      </div>
                    </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <div className="bg-slate-50 rounded-3xl p-8 border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
              <div
                className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform ${activeTab === 'candidate' ? 'bg-purple-100 text-purple-600' : 'bg-emerald-100 text-emerald-600'}`}
              >
            ...
            {/* Feature 2 */}
            <div
              className={`rounded-3xl p-8 border hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group relative overflow-hidden ${activeTab === 'candidate' ? 'bg-purple-50 border-purple-100' : 'bg-emerald-50 border-emerald-100'}`}
            >
              <div
                className={`absolute top-0 right-0 text-white text-[10px] font-bold px-3 py-1 rounded-bl-xl ${activeTab === 'candidate' ? 'bg-purple-600' : 'bg-emerald-600'}`}
              >
                CORE
              </div>
              <div
              ...
              <h4
                className={`text-xl font-bold mb-3 ${activeTab === 'candidate' ? 'text-purple-900' : 'text-emerald-900'}`}
              >
            ...
            <div className="bg-slate-50 rounded-3xl p-8 border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
              <div
                className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform ${activeTab === 'candidate' ? 'bg-green-100 text-green-600' : 'bg-blue-100 text-blue-600'}`}
              >
      ...
      >
        <div
          className={`absolute top-0 left-0 w-full h-full bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] ${activeTab === 'candidate' ? 'from-purple-900/40' : 'from-emerald-900/40'} via-slate-900 to-slate-900 transition-colors duration-500`}
        ></div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="max-w-lg mx-auto bg-white text-slate-900 rounded-3xl p-2 shadow-2xl overflow-hidden transform hover:scale-[1.01] transition-transform duration-300">
            <div
              className={`text-white text-center py-2 text-xs font-bold uppercase tracking-widest rounded-t-2xl ${activeTab === 'candidate' ? 'bg-purple-600' : 'bg-emerald-600'}`}
            >
                ...
                {activeTab === 'candidate' ? (
                  ...
                  <>
                    <li className="flex gap-3 text-slate-700">
                      ...
                      <span>
                        <strong>Diagnóstico IA</strong> ilimitado
                      </span>
                    </li>
                    <li className="flex gap-3 text-slate-700">
                      <CheckCircle2 className="text-purple-500 shrink-0" />
                      <span>
                        Otimizador de <strong>Vagas Externas</strong>
                      </span>
                    </li>
                  ...
                  <>
                    <li className="flex gap-3 text-slate-700">
                      ...
                      <span>
                        Triagem de <strong>Base Ilimitada</strong>
                      </span>
                    </li>
                    <li className="flex gap-3 text-slate-700">
                      <CheckCircle2 className="text-emerald-500 shrink-0" />
            ...
            {faqs.map((faq, index) => (
              ...
              >
                ...
                >
                  ...
                  {openFaqIndex === index ? (
                    <ChevronUp className={theme.text} />
                  ) : (
                    <ChevronDown className="text-slate-400" />
                  )}
                </button>
                <div
                  className={`px-6 pb-6 text-slate-600 leading-relaxed border-t border-slate-50 ${openFaqIndex === index ? 'block' : 'hidden'}`}
                >
                  {faq.answer}
                </div>
              </div>
      ...
      <footer className="bg-slate-950 text-slate-400 py-16 border-t border-slate-900">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-4 gap-12 mb-12">
            <div className="col-span-1 md:col-span-1">
              ...
              <p className="text-sm leading-relaxed mb-4">
                Não somos agência de emprego. Somos uma empresa de tecnologia que usa Inteligência
                Artificial para aumentar a empregabilidade de profissionais e a eficiência de
                empresas.
              </p>
            </div>
            <div>
              ...
              <ul className="space-y-2 text-sm">
                ...
                <li>
                  <Link to="/candidate" className="hover:text-purple-400 transition-colors">
                    Login Candidato
                  </Link>
                </li>
              </ul>
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm">
                <li>
                  <Link to="/sobre" className="hover:text-purple-400 transition-colors">
                    Sobre Nós
                  </Link>
                </li>
                <li>
                ...
                <li>
                  <Link to="/recruiter" className="hover:text-purple-400 transition-colors">
                    Área do Recrutador
                  </Link>
                </li>
              </ul>
          ...
          <div className="pt-8 border-t border-slate-900 text-center text-xs text-slate-600">
            © 2026 Recruta.AI Tecnologia Ltda. Todos os direitos reservados. CNPJ:
            00.000.000/0001-00.
          </div>
        </div>
      </footer>
