...
const PendingReviews = () => {
  ...
  const pendingCandidates = MOCK_CANDIDATES.filter(
    (c) => c.status === 'processing' || c.status === 'error'
  );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 text-sm">
          Candidatos que precisam de análise manual ou tiveram problemas no processamento.
        </p>
      </div>
      ...
      <div className="grid gap-4">
        {pendingCandidates.map((candidate) => (
          <div
            key={candidate.id}
            className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row gap-6"
          >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      Iniciado em {candidate.date} • {candidate.phone}
                    </p>
                  </div>
                </div>
                <div
