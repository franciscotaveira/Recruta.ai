...
vi.mock('../storage/db', () => {
  ...
  return {
    wa: {
      ...
      getSessionByPhone: vi.fn(async (phone) => Object.values(sessions).find(s => s.candidate_phone === phone)),
      updateSessionState: vi.fn(async (state, id) => { if(sessions[id]) sessions[id].state = state; }),
      createSession: vi.fn(async (id, phone, name, jobId, recruiterId) => {
        sessions[id] = { id, candidate_phone: phone, candidate_name: name, job_id: jobId, recruiter_id: recruiterId, state: 'invited' };
      }),
      logMessage: vi.fn(),
...
vi.mock('../ai/sentiment', () => ({
  analyzeResponseRealtime: vi.fn(async () => ({ sentiment: 'neutral', clarity: 1, keyInsights: [] }))
}));
